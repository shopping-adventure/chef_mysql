...
if (node['mysql']['server']['type'] =~ /percona/)
  ...
  lbnode = search(:node, "role:#{node['haproxy']['lb_role']} AND chef_environment:#{node.chef_environment}").first
  template '/etc/xinetd.d/mysqlchk' do
    ...
    only_if "dpkg -l|grep percona-xtradb-cluster-server"
  end

  sqlreq = "USE percona;\n DELETE FROM dsns; \nINSERT into dsns (dsn) value ('h=#{node['cloud']['local_ipv4']},P=#{node['mysql']['port']},u=root,p=#{node['mysql']['server_root_password']}');\n"
  ...
  if (node["mysql"]["percona"]["percona_cluster"] == "init")
    ...
    if (cluster_node.nil? or cluster_node == []) 
      ...
      (cluster_node.sort).each do |n|
        ...
        localport = n['mysql']['port']
        localpass = n['mysql']['server_root_password']
        sqlreq = sqlreq +"INSERT into dsns (dsn) value ('h=#{localip},P=#{localport},u=root,p=#{localpass}');\n"
        if counter == 1
  ...
  #We set the percona.dsns table
  bash 'set dsns table' do
    cwd '/root'
    user 'root'
    code  "echo \"#{sqlreq}\" > /tmp/sqlreq ; mysql -u root < /tmp/sqlreq"
    action :nothing
  end
else
  apt_repository 'percona' do
...
template '/etc/mysql/my.cnf' do
  ...
  if (node['mysql']['server']['type'] == "percona-cluster")
    notifies :run, 'bash[set dsns table]'
  end
...
if (node['mysql']['server']['type'] =~ /percona/)
  ...
  # Install dsn table for checksumming/monitoring
  template '/etc/mysql_percona-dsn.sql' do
    source 'percona-dsn.sql.erb'
    owner 'root'
    group 'root'
    mode '0644'
    notifies :run, 'execute[install-dsn]', :immediately
  end

  dsn = install_dsn_cmd
  execute 'install-dsn' do
    command dsn
    action :nothing
  end

  # For pt-table-checksum run by zabbix
  bash 'set_perm_tmp_percona_version' do
    user 'root'
    code 'chmod o+r /tmp/percona-version-check'
    action :nothing
    only_if { ::File.exists?('/tmp/percona-version-check') }
  end
end
service 'mysql' do
  service_name 'mysql'
  ...
  action       [:enable, :start]
  if (node['mysql']['server']['type'] == "percona-cluster")
    notifies :run, 'bash[set_perm_tmp_percona_version]'
  end
end
